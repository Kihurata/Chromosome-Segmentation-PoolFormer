...
- [x] **Code Implementation:** Update `src/stage2_cls/train_cluster.py` and `configs/stage2_cls.yaml`.
- [x] **Dry Run:** Verify if `StarReLU` and `MLN` load correctly.
- [x] **Monitor:** Check Confusion Matrix at Epoch 20 to compare with previous baseline.

---

## 🚦 Verification Checklist
- [x] Does it crash with OOM on Batch Size 32 effective?
- [x] Is the Warmup visible in the LR curve in `scalars.json`?
- [x] Is the "Touching" class performance improving in validation?
